...
CONST uint8 EpdBattUUID[ATT_BT_UUID_SIZE] = {
    ...
    HI_UINT16(EPD_BATT_UUID),
};

CONST uint8 EpdTempUUID[ATT_BT_UUID_SIZE] = {
    LO_UINT16(EPD_TEMP_UUID),
...
CONST uint8 EpdRxTxUUID[ATT_BT_UUID_SIZE] = {
    ...
    HI_UINT16(EPD_RXTX_UUID),
};

// meter
const uint8 EpdRetainDistanceUUID[ATT_BT_UUID_SIZE] = {
    LO_UINT16(EPD_RETAIN_DISTANCE_UUID),
    HI_UINT16(EPD_RETAIN_DISTANCE_UUID),
};
...
// meter
static uint8 RetainDistanceProps = GATT_PROP_READ | GATT_PROP_WRITE;
...
static gattAttribute_t EpdServiceAttrTbl[] = {
    ...
    // Characteristic Value
    {{ATT_BT_UUID_SIZE, EpdBattUUID}, GATT_PERMIT_READ, 0, NULL},

    // Characteristic Declaration
    {{ATT_BT_UUID_SIZE, characterUUID}, GATT_PERMIT_READ, 0, &TempProps},
    // Characteristic Value
    ...
    {{ATT_BT_UUID_SIZE, characterUUID},
     ...
     &RetainDistanceProps},
    // Characteristic Value
    {{ATT_BT_UUID_SIZE, EpdRetainDistanceUUID},
     GATT_PERMIT_READ | GATT_PERMIT_WRITE,
     0,
     NULL},
};
...
bStatus_t EPDService_AddService(uint8_t rspTaskId) {
  ...
  status = GATTServApp_RegisterService(
      EpdServiceAttrTbl, GATT_NUM_ATTRS(EpdServiceAttrTbl),
      GATT_MAX_ENCRYPT_KEY_SIZE, &EpdServiceCBs);
  return status;
}
...
static bStatus_t EPDService_ReadAttrCB(uint16_t connHandle,
  ...
  case EPD_RETAIN_DISTANCE_UUID: {
    *pLen = sizeof(retainDistance);
    memcpy(pValue, &retainDistance, sizeof(retainDistance));
    break;
  }
...
static bStatus_t EPDService_WriteAttrCB(uint16_t connHandle,
  ...
  case EPD_RETAIN_DISTANCE_UUID: {
    if (len == sizeof(retainDistance)) {
      retainDistance = *(uint16_t *)pValue;
  ...
  if (changeParamID != 0xFF) {
    if (pAppCBs && pAppCBs->pfnChangeCb) {
      pAppCBs->pfnChangeCb(
          connHandle, changeParamID, len + offset,
          pValue); // Call app function from stack task context.
    }
  }
  return status;
}
