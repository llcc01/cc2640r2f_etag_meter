...
static PIN_Config GPIOTable[] = {
    // EPD_POWER_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
    // PIN_DRVSTR_MIN,
    EPD_BUSY_PIN | PIN_GPIO_OUTPUT_DIS | PIN_INPUT_EN | PIN_PULLUP,
    EPD_DC_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_RST_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_CS_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_SCL_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_SDA_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_LED1_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    EPD_LED2_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MIN,
    PIN_TERMINATE};
...
uint16_t epd_battery = (uint16_t)-1;
...
static inline void DEV_Delay_ms(uint16_t t) { Util_delay_ms(t); }

static inline void DEV_Delay_us(uint16_t t) { CPUdelay(t); }
...
uint8_t EPD_SSD_ReadData() {
  ...
  PIN_setConfig(GPIOHandle, PIN_BM_ALL,
                EPD_SDA_PIN | PIN_GPIO_OUTPUT_DIS | PIN_INPUT_EN | PIN_PULLUP);
  DEV_Digital_Write(EPD_DC_PIN, 1);
  DEV_Digital_Write(EPD_CS_PIN, 0);
  ...
  PIN_setConfig(GPIOHandle, PIN_BM_ALL,
                EPD_SDA_PIN | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH |
...
void LED_On(uint8_t io) { DEV_Digital_Write(io, 0); }

void LED_Off(uint8_t io) { DEV_Digital_Write(io, 1); }
...
int8_t RTC_GetCollaborate(void) { return epd_rtc_collab; }
#endif
...
uint8_t u8x8_cc2640_gpio_and_delay(U8X8_UNUSED u8x8_t *u8x8,
  ...
  case U8X8_MSG_GPIO_SPI_DATA:
    DEV_Digital_Write(EPD_SDA_PIN, arg_int);
    break;
  case U8X8_MSG_GPIO_SPI_CLOCK:
    DEV_Digital_Write(EPD_SCL_PIN, arg_int);
    break;
  case U8X8_MSG_GPIO_CS:
    DEV_Digital_Write(EPD_CS_PIN, arg_int);
    break;
  case U8X8_MSG_GPIO_DC:
    DEV_Digital_Write(EPD_DC_PIN, arg_int);
    break;
  case U8X8_MSG_GPIO_RESET:
    DEV_Digital_Write(EPD_RST_PIN, arg_int);
    break;
